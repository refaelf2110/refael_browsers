// Mini execution: 3 versions (oldest / middle / latest) per category, headless + headfull.
...
// ── Exit diagnostics ──────────────────────────────────────────────────────────
{
  ...
  process.exit = function diagExit(code) {
    try { _fsSync.appendFileSync(_dbgLog, `[${new Date().toISOString()}] process.exit(${code})\n${new Error().stack}\n`, 'utf8'); } catch(_) {}
    _origExit(code);
  };
  process.on('exit', code => {
    try { _fsSync.appendFileSync(_dbgLog, `[${new Date().toISOString()}] exit event code=${code}\n`, 'utf8'); } catch(_) {}
  });
}
...
// Remove Taiko's built-in 'exitOnUnhandledFailures' handler that calls process.exit(1).
...
const RESULTS_FILE = path.join(CACHE_DIR, 'results_mini.html');
...
const EXTRACTOR_TIMEOUT      = 8 * 60 * 1000;  // 8 min — get_vars blocks for minutes on large windows
const EXTRACTOR_TASK_TIMEOUT = 15 * 60 * 1000; // 15 min per task incl. browser launch + data transfer
...
async function withConcurrency(fns, limit, taskTimeoutMs = TASK_TIMEOUT_MS) {
  let i = 0;
  async function worker() {
    while (i < fns.length) {
      ...
      await Promise.race([
        fn().catch(() => {}),
        new Promise(r => setTimeout(r, taskTimeoutMs)),
      ]);
    }
  }
...
async function runPuppeteerFirefoxTest(major, ffPath, headless) {
  ...
  try {
    ...
    // evaluateOnNewDocument (BiDi: script.addPreloadScript) injects x-refael into XHR/fetch BEFORE page scripts run.
    await page.setExtraHTTPHeaders({ 'x-refael': '7e8afcbdd3' })
      .then(() => console.log(`[pup-ff-${major}] setExtraHTTPHeaders OK`))
      .catch(e => console.log(`[pup-ff-${major}] setExtraHTTPHeaders failed:`, e.message.split('\n')[0]));
    await gotoWithRetry(page, TEST_URL, { waitUntil: 'domcontentloaded', timeout: NAV_TIMEOUT * 2 });
    // addScriptTag runs in the page's JS world (unlike evaluateOnNewDocument which is isolated in BiDi).
    await page.addScriptTag({ content:
      '(function(){var H="x-refael",V="7e8afcbdd3";' +
      'var oSend=XMLHttpRequest.prototype.send;' +
      'XMLHttpRequest.prototype.send=function(){try{this.setRequestHeader(H,V);}catch(e){}return oSend.apply(this,arguments);};' +
      'var oFetch=window.fetch;' +
      'window.fetch=function(input,init){init=Object.assign({},init||{});if(init.mode==="no-cors")return oFetch.call(window,input,init);' +
      'var h=init.headers;if(h&&typeof h.set==="function"){h.set(H,V);}else{init.headers=Object.assign({},h||{});init.headers[H]=V;}' +
      'return oFetch.call(window,input,init);};})();'
    }).catch(e => console.log(`[pup-ff-${major}] addScriptTag failed:`, e.message.split('\n')[0]));
    const reasonReceived = await page.waitForFunction(REASON_READY_JS, { timeout: SYNC_TIMEOUT, polling: 500 }).then(() => true).catch(() => false);
    const rawList = await page.evaluate(EXTRACT_REASONS_JS).catch(() => null);
    positiveReasons = extractPositives(rawList);
    allReasons      = extractAllReasons(rawList);
...
// ── Cypress ───────────────────────────────────────────────────────────────────
// Cypress uses its built-in proxy to intercept requests; cy.intercept injects headers.
...
// ── Extractor: shared wait+collect helper for Playwright / Puppeteer pages ────

async function extractFromPage(page, browserLabel) {
  ...
  if (vars.length > 0) saveWindowElements(browserLabel, vars);
  else console.log(`[extractor] ${browserLabel}: no data collected`);
}

// ── Extractor: per-framework runners ──────────────────────────────────────────

async function runExtractorPlaywright(label, major, engine, launchOptions, headless) {
  const mode = headless ? 'headless' : 'headfull';
  let browser = null, resolvedMajor = major ?? '?';
  try {
    browser = await engine.launch({ ...launchOptions, headless });
    if (!major) resolvedMajor = browser.version().split('.')[0];
    const page = await browser.newPage();
    await page.goto(EXTRACTOR_URL, { waitUntil: 'domcontentloaded', timeout: NAV_TIMEOUT });
    await extractFromPage(page, `playwright-${label}-${resolvedMajor}-${mode}`);
  } catch(err) {
    console.error(`[extractor] playwright-${label}-${resolvedMajor}-${mode}: ${err.message.split('\n')[0]}`);
  } finally {
    if (browser) await Promise.race([browser.close(), new Promise(r => setTimeout(r, 10000))]).catch(() => {});
  }
}

async function runExtractorPuppeteerChrome(major, executablePath, headless) {
  const mode = headless ? 'headless' : 'headfull';
  ...
  try {
    ...
    console.error(`[extractor] ${lbl}: ${err.message.split('\n')[0]}`);
  } finally {
    if (browser) await Promise.race([browser.close(), new Promise(r => setTimeout(r, 10000))]).catch(() => {});
  }
}

async function runExtractorPuppeteerFirefox(major, ffPath, headless) {
  const mode = headless ? 'headless' : 'headfull';
  const lbl  = `puppeteer-firefox-${major}-${mode}`;
  let browser = null;
  try {
    browser = await puppeteer.launch({ executablePath: ffPath, browser: 'firefox', headless, timeout: 60000 });
    const page = await browser.newPage();
    await page.goto(EXTRACTOR_URL, { waitUntil: 'domcontentloaded', timeout: NAV_TIMEOUT });
    await extractFromPage(page, lbl);
  } catch(err) {
...
async function runExtractorPuppeteerEdge(label, major, edgePath, headless) {
  const mode = headless ? 'headless' : 'headfull';
  const lbl  = `puppeteer-${label}-${major}-${mode}`;
  let browser = null;
  try {
    browser = await puppeteer.launch({ executablePath: edgePath, headless, args: ['--no-sandbox', '--disable-gpu'] });
    const page = await browser.newPage();
    await page.goto(EXTRACTOR_URL, { waitUntil: 'domcontentloaded', timeout: NAV_TIMEOUT });
    await extractFromPage(page, lbl);
  } catch(err) {
...
// Wait for #extractor-data = 'DONE' inside Selenium via an in-page setTimeout loop.
...
async function runExtractorSeleniumChrome(major, chromePath, driverPath, headless) {
  const mode = headless ? 'headless' : 'headfull';
...
async function runExtractorSeleniumFirefox(major, ffPath, geckodriverPath, headless) {
  const mode = headless ? 'headless' : 'headfull';
...
async function runExtractorWebdriverIOChrome(major, chromePath, driverPath, headless) {
  const mode = headless ? 'headless' : 'headfull';
  ...
  try {
    ...
    // Use Puppeteer interop for page-level evaluate (same pattern as detection tests)
    const pBrowser = await wdioBrowser.getPuppeteer();
    const pages    = await pBrowser.pages();
    const page     = pages[0] || await pBrowser.newPage();
    await page.goto(EXTRACTOR_URL, { waitUntil: 'domcontentloaded', timeout: NAV_TIMEOUT });
...
// Taiko: global browser state — run all sampled versions sequentially in one task.
async function runAllExtractorTaikoTests(chromes) {
  for (const { major, executablePath } of chromes) {
    for (const headless of [true, false]) {
      const mode = headless ? 'headless' : 'headfull';
      const lbl  = `taiko-chromium-${major}-${mode}`;
      try {
        process.env.TAIKO_BROWSER_PATH = executablePath;
        await taiko.openBrowser({ headless, args: ['--no-sandbox', '--disable-gpu'] });
        ...
        while (Date.now() < deadline) {
          const done = await taiko.evaluate(() => {
            var el = document.getElementById('extractor-data');
...
// ── Extractor: build task list (mirrors mini detection sampling) ───────────────

async function buildExtractorTaskList(sampleFn = sample) {
  ...
  // Playwright — same variants as detection
  for (const headless of [true, false])
    tasks.push(() => runExtractorPlaywright('chromium', null, chromium, { args: ['--no-sandbox'] }, headless));
  for (const headless of [true, false])
    tasks.push(() => runExtractorPlaywright('firefox-pw', null, firefox, {}, headless));
  for (const b of sampleFn(chromes)) {
    const major = b.buildId.split('.')[0];
    for (const headless of [true, false])
      tasks.push(() => runExtractorPlaywright('chrome', major, chromium,
        { executablePath: b.executablePath, args: ['--no-sandbox'] }, headless));
  }
  for (const [label, channel] of [['edge','msedge'],['edge-beta','msedge-beta'],['edge-dev','msedge-dev']]) {
    for (const headless of [true, false])
      tasks.push(() => runExtractorPlaywright(label, null, chromium, { channel, args: ['--no-sandbox'] }, headless));
  }

  // Puppeteer Chrome — sampled
  for (const b of sampleFn(chromes)) {
    const major = b.buildId.split('.')[0];
    for (const headless of [true, false])
      tasks.push(() => runExtractorPuppeteerChrome(major, b.executablePath, headless));
  }

  // Puppeteer Firefox — sampled
  const ppFirefoxes = allInstalled
    .filter(b => b.browser === Browser.FIREFOX && fs.existsSync(b.executablePath))
    .sort((a, b) => a.buildId.localeCompare(b.buildId, undefined, { numeric: true }));
  for (const b of sampleFn(ppFirefoxes)) {
    const major = b.buildId.replace(/^[^_]+_/, '').split('.')[0];
    for (const headless of [true, false])
      tasks.push(() => runExtractorPuppeteerFirefox(major, b.executablePath, headless));
  }

  // Puppeteer Edge — all 3 channels
  if (fs.existsSync(EDGE_MANIFEST)) {
    const em = JSON.parse(fs.readFileSync(EDGE_MANIFEST, 'utf8'));
    for (const { label, version, edgePath } of Object.values(em)) {
      if (fs.existsSync(edgePath)) {
        const major = version.split('.')[0];
        for (const headless of [true, false])
          tasks.push(() => runExtractorPuppeteerEdge(label, major, edgePath, headless));
      }
    }
  }

  // Selenium Chrome — sampled
  ...
  for (const b of sampleFn(chromes.filter(c => chromeDriverMap.has(c.buildId)))) {
    const major = b.buildId.split('.')[0];
    for (const headless of [true, false])
      tasks.push(() => runExtractorSeleniumChrome(major, b.executablePath, chromeDriverMap.get(b.buildId), headless));
  }

  // Selenium Firefox — sampled
  if (fs.existsSync(FF_DIR)) {
    const ffDirs = fs.readdirSync(FF_DIR)
      .filter(d => fs.existsSync(path.join(FF_DIR, d, 'firefox.exe')))
      .sort((a, b) => Number(a) - Number(b));
    for (const major of sampleFn(ffDirs)) {
      const exe     = path.join(FF_DIR, major, 'firefox.exe');
      const gecko   = geckoExeFor(major);
      if (!fs.existsSync(gecko)) continue;
      for (const headless of [true, false])
        tasks.push(() => runExtractorSeleniumFirefox(major, exe, gecko, headless));
    }
  }

  // WebdriverIO Chrome — sampled
  for (const b of sampleFn(chromes.filter(c => chromeDriverMap.has(c.buildId)))) {
    const major = b.buildId.split('.')[0];
    for (const headless of [true, false])
      tasks.push(() => runExtractorWebdriverIOChrome(major, b.executablePath, chromeDriverMap.get(b.buildId), headless));
  }

  // Taiko — sampled Chrome versions, all sequential in one task
  const taikoChromes = sampleFn(chromes).map(b => ({ major: b.buildId.split('.')[0], executablePath: b.executablePath }));
  tasks.push(() => runAllExtractorTaikoTests(taikoChromes));

  return tasks;
}

// ── build task list (MINI — oldest / middle / latest per category) ─────────────
...
async function buildTaskList() {
  ...
  // ── Playwright Edge — all 3 channels (already just 3) ─────────────────────
  for (const [label, channel] of [['edge', 'msedge'], ['edge-beta', 'msedge-beta'], ['edge-dev', 'msedge-dev']]) {
    for (const headless of [true, false])
      if (want('playwright', label, null, headless))
        tasks.push(() => runPlaywrightTest(label, null, chromium, { channel, args: ['--no-sandbox', '--no-first-run', '--disable-default-apps', '--no-default-browser-check'] }, headless));
  }

  // ── Puppeteer — 3 sampled Chrome versions ─────────────────────────────────
  for (const b of sample(chromes)) {
    const major = b.buildId.split('.')[0];
    for (const headless of [true, false])
  ...
  // ── Puppeteer Firefox — 3 sampled versions (via @puppeteer/browsers) ───────
  const puppeteerFirefoxes = allInstalled
    .filter(b => b.browser === Browser.FIREFOX && fs.existsSync(b.executablePath))
    .sort((a, b) => a.buildId.localeCompare(b.buildId, undefined, { numeric: true }));
  for (const b of sample(puppeteerFirefoxes)) {
    const major = b.buildId.replace(/^[^_]+_/, '').split('.')[0];
    for (const headless of [true, false])
  ...
  // ── Selenium Chrome — 3 sampled versions ──────────────────────────────────
  const chromeDriverMap = new Map(
    allInstalled.filter(b => b.browser === Browser.CHROMEDRIVER && fs.existsSync(b.executablePath))
      .map(b => [b.buildId, b.executablePath])
  );
  for (const b of sample(chromes.filter(c => chromeDriverMap.has(c.buildId)))) {
    const major = b.buildId.split('.')[0];
    for (const headless of [true, false])
  ...
  // ── Selenium Firefox — 3 sampled versions ─────────────────────────────────
  if (fs.existsSync(FF_DIR)) {
    const allDirs = fs.readdirSync(FF_DIR)
      .filter(d => fs.existsSync(path.join(FF_DIR, d, 'firefox.exe')))
      .sort((a, b) => Number(a) - Number(b));
    for (const major of sample(allDirs)) {
      const exe = path.join(FF_DIR, major, 'firefox.exe');
      const geckoExe = geckoExeFor(major);
      if (!fs.existsSync(geckoExe)) continue;
      for (const headless of [true, false])
  ...
  // ── WebdriverIO Chrome — 3 sampled versions ───────────────────────────────
  for (const b of sample(chromes.filter(c => chromeDriverMap.has(c.buildId)))) {
    const major = b.buildId.split('.')[0];
    for (const headless of [true, false])
  ...
  // Use NSIS-installed versions from FF_DIR which Selenium confirms can create sessions.
  if (fs.existsSync(FF_DIR)) {
    const wdioFirefoxFiltered = sample(
      fs.readdirSync(FF_DIR)
        .sort((a, b) => Number(a) - Number(b))
        .filter(major => {
          const exe = path.join(FF_DIR, major, 'firefox.exe');
  ...
  // ── Taiko — 3 sampled Chrome for Testing versions (sequential: global browser state)
...
async function run() {
  ...
  console.log(`[MINI] Running ${tasks.length} combinations (${CONCURRENCY} concurrent)...\n`);
...
async function runExtractorMini() {
  const server = startExtractorServer();
  try {
    const tasks = await buildExtractorTaskList(sample2);
    console.log(`[EXTRACTOR-MINI] Running ${tasks.length} tasks (${CONCURRENCY} concurrent)...\n`);
    const startMs = Date.now();
    await withConcurrency(tasks, CONCURRENCY, EXTRACTOR_TASK_TIMEOUT);
    const elapsed = formatElapsed(Date.now() - startMs);
    console.log(`\n[EXTRACTOR-MINI] Done in ${elapsed}`);
  } finally {
    server.close();
  }
}

if (process.env.EXTRACTOR_ONLY === '1') {
  runExtractorMini().catch(err => { console.error(err); process.exit(1); });
} else {
