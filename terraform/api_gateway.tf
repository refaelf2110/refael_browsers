...
resource "aws_api_gateway_resource" "browsers" {
  rest_api_id = aws_api_gateway_rest_api.refael.id
  parent_id   = aws_api_gateway_rest_api.refael.root_resource_id
  path_part   = "browsers"
}

resource "aws_api_gateway_resource" "browsers_available" {
  rest_api_id = aws_api_gateway_rest_api.refael.id
  parent_id   = aws_api_gateway_resource.browsers.id
  path_part   = "available"
}

resource "aws_api_gateway_method" "browsers_available_get" {
  rest_api_id   = aws_api_gateway_rest_api.refael.id
  resource_id   = aws_api_gateway_resource.browsers_available.id
  http_method   = "GET"
  authorization = "NONE"
}

resource "aws_api_gateway_integration" "browsers_available_get" {
  rest_api_id             = aws_api_gateway_rest_api.refael.id
  resource_id             = aws_api_gateway_resource.browsers_available.id
  http_method             = aws_api_gateway_method.browsers_available_get.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.refael_results_api.invoke_arn
}

resource "aws_api_gateway_method" "browsers_available_options" {
  rest_api_id   = aws_api_gateway_rest_api.refael.id
  resource_id   = aws_api_gateway_resource.browsers_available.id
  http_method   = "OPTIONS"
  authorization = "NONE"
...
resource "aws_api_gateway_integration" "browsers_available_options" {
  rest_api_id = aws_api_gateway_rest_api.refael.id
  resource_id = aws_api_gateway_resource.browsers_available.id
  http_method = aws_api_gateway_method.browsers_available_options.http_method
  type        = "MOCK"
  request_templates = {
    "application/json" = "{\"statusCode\": 200}"
  }
...
resource "aws_api_gateway_method_response" "browsers_available_options_200" {
  rest_api_id = aws_api_gateway_rest_api.refael.id
  resource_id = aws_api_gateway_resource.browsers_available.id
  http_method = aws_api_gateway_method.browsers_available_options.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "browsers_available_options" {
  rest_api_id = aws_api_gateway_rest_api.refael.id
  resource_id = aws_api_gateway_resource.browsers_available.id
  http_method = aws_api_gateway_method.browsers_available_options.http_method
  status_code = aws_api_gateway_method_response.browsers_available_options_200.status_code
  response_parameters = local.cors_headers
  depends_on  = [aws_api_gateway_integration.browsers_available_options]
}

# /browsers/new
resource "aws_api_gateway_resource" "browsers_new" {
  rest_api_id = aws_api_gateway_rest_api.refael.id
  parent_id   = aws_api_gateway_resource.browsers.id
  path_part   = "new"
}

resource "aws_api_gateway_method" "browsers_new_get" {
  rest_api_id   = aws_api_gateway_rest_api.refael.id
  resource_id   = aws_api_gateway_resource.browsers_new.id
  http_method   = "GET"
  authorization = "NONE"
}

resource "aws_api_gateway_integration" "browsers_new_get" {
  rest_api_id             = aws_api_gateway_rest_api.refael.id
  resource_id             = aws_api_gateway_resource.browsers_new.id
  http_method             = aws_api_gateway_method.browsers_new_get.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.refael_results_api.invoke_arn
}

resource "aws_api_gateway_method" "browsers_new_options" {
  rest_api_id   = aws_api_gateway_rest_api.refael.id
  resource_id   = aws_api_gateway_resource.browsers_new.id
  http_method   = "OPTIONS"
  authorization = "NONE"
...
resource "aws_api_gateway_integration" "browsers_new_options" {
  rest_api_id = aws_api_gateway_rest_api.refael.id
  resource_id = aws_api_gateway_resource.browsers_new.id
  http_method = aws_api_gateway_method.browsers_new_options.http_method
  type        = "MOCK"
  request_templates = {
    "application/json" = "{\"statusCode\": 200}"
  }
...
resource "aws_api_gateway_method_response" "browsers_new_options_200" {
  rest_api_id = aws_api_gateway_rest_api.refael.id
  resource_id = aws_api_gateway_resource.browsers_new.id
  http_method = aws_api_gateway_method.browsers_new_options.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "browsers_new_options" {
  rest_api_id = aws_api_gateway_rest_api.refael.id
  resource_id = aws_api_gateway_resource.browsers_new.id
  http_method = aws_api_gateway_method.browsers_new_options.http_method
  status_code = aws_api_gateway_method_response.browsers_new_options_200.status_code
  response_parameters = local.cors_headers
  depends_on  = [aws_api_gateway_integration.browsers_new_options]
}
...
# /dashboard/combined
resource "aws_api_gateway_resource" "dashboard_combined" {
  rest_api_id = aws_api_gateway_rest_api.refael.id
  parent_id   = aws_api_gateway_resource.dashboard.id
  path_part   = "combined"
}

resource "aws_api_gateway_method" "dashboard_combined_get" {
  rest_api_id   = aws_api_gateway_rest_api.refael.id
  resource_id   = aws_api_gateway_resource.dashboard_combined.id
  http_method   = "GET"
  authorization = "NONE"
}

resource "aws_api_gateway_integration" "dashboard_combined_get" {
  rest_api_id             = aws_api_gateway_rest_api.refael.id
  resource_id             = aws_api_gateway_resource.dashboard_combined.id
  http_method             = aws_api_gateway_method.dashboard_combined_get.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.refael_results_api.invoke_arn
}

resource "aws_api_gateway_method" "dashboard_combined_options" {
  rest_api_id   = aws_api_gateway_rest_api.refael.id
  resource_id   = aws_api_gateway_resource.dashboard_combined.id
  http_method   = "OPTIONS"
  authorization = "NONE"
...
resource "aws_api_gateway_integration" "dashboard_combined_options" {
  rest_api_id = aws_api_gateway_rest_api.refael.id
  resource_id = aws_api_gateway_resource.dashboard_combined.id
  http_method = aws_api_gateway_method.dashboard_combined_options.http_method
  type        = "MOCK"
  request_templates = {
    "application/json" = "{\"statusCode\": 200}"
  }
...
resource "aws_api_gateway_method_response" "dashboard_combined_options_200" {
  rest_api_id = aws_api_gateway_rest_api.refael.id
  resource_id = aws_api_gateway_resource.dashboard_combined.id
  http_method = aws_api_gateway_method.dashboard_combined_options.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "dashboard_combined_options" {
  rest_api_id = aws_api_gateway_rest_api.refael.id
  resource_id = aws_api_gateway_resource.dashboard_combined.id
  http_method = aws_api_gateway_method.dashboard_combined_options.http_method
  status_code = aws_api_gateway_method_response.dashboard_combined_options_200.status_code
  response_parameters = local.cors_headers
  depends_on  = [aws_api_gateway_integration.dashboard_combined_options]
}
...
resource "aws_api_gateway_deployment" "refael" {
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_resource.browsers_available.id,
      aws_api_gateway_method.browsers_available_get.id,
      aws_api_gateway_integration.browsers_available_get.id,
      aws_api_gateway_resource.browsers_new.id,
      aws_api_gateway_method.browsers_new_get.id,
      aws_api_gateway_integration.browsers_new_get.id,
      aws_api_gateway_resource.dashboard_combined.id,
      aws_api_gateway_method.dashboard_combined_get.id,
      aws_api_gateway_integration.dashboard_combined_get.id,
    ]))
  }
  ...
  depends_on = [
    ...
    aws_api_gateway_integration.browsers_available_get,
    aws_api_gateway_integration.browsers_available_options,
    aws_api_gateway_integration.browsers_new_get,
    aws_api_gateway_integration.browsers_new_options,
    aws_api_gateway_integration.dashboard_combined_get,
    aws_api_gateway_integration.dashboard_combined_options,
  ]
}
