...
/**
 * sync_browsers_from_s3_linux.js
 *
 * Downloads browser binaries from S3 into /browsers at container startup.
 *
 ...
 *   BROWSERS_CACHE_LINUX_PREFIX    — S3 prefix    (e.g. linux/)
...
const PREFIX      = process.env.BROWSERS_CACHE_LINUX_PREFIX || 'linux/';
const DEST_DIR    = '/browsers';
const CONCURRENCY = 8;
...
// VERSION_LIST: { chrome: ["131.0.6778.87", ...], firefox: ["132", ...] }
...
(async () => {
  ...
  const fns = objects.map(({ key, size, rel }) => {
    const localPath = path.join(DEST_DIR, rel);
    return async () => {
